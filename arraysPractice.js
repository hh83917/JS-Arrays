...
//Write a function called reversedLooper that is given letters as it's only argument and loops through the given array backwards alerting every item in the array starting at the end.
  ...
  //Code Here
  function reversedLopper(letters) {
    for (var i = 0; i < letters.length; i++) {
      alert(letters.reverse()[i]);
    }
  }

  reversedLopper(letters);
...
//Write a function called maker that creates an array, fills that array with numbers from 1 to 215, then returns the array.
  ...
  function maker() {
    var numbers = [];
    for (var i = 0; i < 215; i++) {
      var num = Math.floor(Math.random() * (215 - 1 + 1) + 1);
      if (numbers[i] !== num) {
        numbers.push(num);
      }
    }
...
function addTen(numbers) {
  for (i in numbers) {
    numPlus.push(Number(numbers[i]) + 10);
  }
}
...
Loop through your devMountainEmployees until you find cahlan, then remove him from the array.*/
  ...
  //Code Here
function findCahlan() {
  for (prop in devMountainEmployees) {
    if (devMountainEmployees[prop] === cahlan) {
      delete devMountainEmployees[prop];
    }
  }
}
...
Once you find the particular index he's located in, delete him from the array.*/
  ...
  function findTyler() {
    for (prop in users) {
      if (users[prop].email === 'tylermcginnis33@gmail.com') {
        delete users[prop];
      }
    }
  }
